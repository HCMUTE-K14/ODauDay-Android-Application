...
public class FavoriteTabMainFragment extends BaseMVVMFragment<FragmentFavoriteTabMainBinding> implements FavoriteContract {
    ...
    HeaderFavoriteView.OnClickShareListener mOnClickShareListener=view -> {
        ...
        mFragmentDialogFavoriteShare=new FragmentDialogFavoriteShare("Quoc", "kunsubin96@gmail.com");
    ...
    private List<Property> sortLastAdded(List<Property> list){
        List<Property> propertyList=list;
        Comparator<Property> comparator=(property1, property2) -> {
            if (property1.getFavorite().getDate_created().after(property2.getFavorite().getDate_created())) return -1;
            if (property1.getFavorite().getDate_created().before(property2.getFavorite().getDate_created())) return 1;
            return 0;
        };
        Collections.sort(propertyList,comparator);
        return propertyList;
