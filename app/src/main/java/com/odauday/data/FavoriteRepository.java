...
public class FavoriteRepository implements Repository {
    ...
    public Single<FavoriteResponse> getFavoritePropertyByUser(String user_id) {
        Single<JsonResponse<FavoriteResponse>> result = mFavoriteService.getFavoritePropertyByUser(user_id);
        return result
            .map(response -> {
                try {
    ...
    public Single<MessageResponse> shareFavorite(ShareFavorite shareFavorite) {
        Single<JsonResponse<MessageResponse>> result = mFavoriteService.shareFavorite(shareFavorite);
        return result
            .map(response -> {
                try {
                    if(response.isSuccess()){
